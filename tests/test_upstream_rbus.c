...
{
	int result = rbusRegCloudConnectionOnlineEvent();
...
{
	int result = rbusRegCloudConnectionOnlineEvent();
	result = rbusRegCloudConnectionOnlineEvent();
	CU_ASSERT_NOT_EQUAL(result, 0);
}

//Test function for CloudConnectionSubscribeHandler success
void test_CloudConnectionSubscribeHandler_success()
{
	rbusError_t ret = RBUS_ERROR_BUS_ERROR;
	ret = CloudConnectionSubscribeHandler(handle, RBUS_EVENT_ACTION_SUBSCRIBE , CLOUD_CONN_ONLINE, NULL, 0, false);
...
{
    ParodusInfo("Inside subscribeEventSuccessCallbackHandler\n");
...
void add_suites( CU_pSuite *suite )
{
	*suite = CU_add_suite( "tests", NULL, NULL );
	CU_add_test( *suite, "test rbusRegCloudConnectionOnlineEventRbushandle_failure", test_rbusRegCloudConnectionOnlineEventRbushandle_failure);
	CU_add_test( *suite, "test rbusRegCloudConnectionOnlineEvent_success", test_rbusRegCloudConnectionOnlineEvent_success);
	CU_add_test( *suite, "test rbusRegCloudConnectionOnlineEvent_failure", test_rbusRegCloudConnectionOnlineEvent_failure);
	CU_add_test( *suite, "test CloudConnectionSubscribeHandler_success", test_CloudConnectionSubscribeHandler_success);
